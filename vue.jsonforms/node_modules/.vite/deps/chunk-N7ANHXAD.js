...
var filterOperators = function(operator, settings) {
  return {
    contains: stringFnOperator(__assign({}, settings, { operator: "contains" })),
    doesnotcontain: compose(appendEqual, stringFnOperator(__assign({}, settings, { operator: "indexof" }))),
    endswith: stringFnOperator(__assign({}, settings, { operator: "endswith" })),
    eq: typedOperator(__assign({}, settings, { operator: "eq" })),
    gt: typedOperator(__assign({}, settings, { operator: "gt" })),
    gte: typedOperator(__assign({}, settings, { operator: "ge" })),
    ...
    lt: typedOperator(__assign({}, settings, { operator: "lt" })),
    lte: typedOperator(__assign({}, settings, { operator: "le" })),
    neq: typedOperator(__assign({}, settings, { operator: "ne" })),
    startswith: stringFnOperator(__assign({}, settings, { operator: "startswith" }))
  }[operator];
};
var join = function(x) {
  return " " + x.logic + " ";
...
var toObject = function(values) {
  return values.reduce(function(acc, _a) {
    var key = _a[0], value2 = _a[1];
    var _b;
    return __assign({}, acc, (_b = {}, _b[key] = value2, _b));
  }, {});
};
...
var merge = function(data, left, middle, right, compare2) {
  ...
  var dest = left;
  do {
    if (compare2(data[cursor2], temp[cursor1]) < 0) {
...
var sort2 = function(data, start, end, compare2) {
  if (end - start < 2) {
    return;
  }
  var mid = start + end >>> 1;
  sort2(data, start, mid, compare2);
  sort2(data, mid, end, compare2);
  merge(data, start, mid, end, compare2);
};
...
var orderBy = function(data, descriptors) {
  if (descriptors.some(function(x) {
    ...
    sort2(data, 0, data.length, comparer);
  }
  return data;
};
...
//# sourceMappingURL=chunk-N7ANHXAD.js.map

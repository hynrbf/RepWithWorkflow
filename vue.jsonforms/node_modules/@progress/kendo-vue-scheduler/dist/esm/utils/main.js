...
import { toGroupResources, toFlatGroupResources } from '../views/common/utils.js';
import { getter, setter, clone, cloneDate } from '@progress/kendo-vue-common';
import { 
// OCCURRENCE_ID,
DEFAULT_ID } from '../constants/main.js';
    ...
    slots.forEach(function (slot) {
        ...
        sorted.forEach(function (item) {
            var fit = (ignoreIsAllDay || (item.$props.isAllDay === slot.cSlot.isAllDay))
                ...
                && intersects(item.$props.start, item.$props.end, slot.cSlot.start, slot.cSlot.end);
            if (fit) {
    ...
    slots.forEach(function (slot) {
        ...
        items.forEach(function (item) {
            ...
            var fit = (ignoreIsAllDay || (item.isAllDay === currentSlot.isAllDay))
                ...
                && intersects(item.start, item.end, currentSlot.start, currentSlot.end);
            if (fit) {
                var order = findMissing(orderNumbers);
...
/** @hidden */
export var compareItems = function (f, s, ignoreIsAllDay) { return Boolean(f.uid === s.uid
...
/** @hidden */
export var getSortedSlots = function (all, ignoreIsAllDay) {
...
/** @hidden */
export var compareSlots = function (f, s) { return Boolean(f.start.getTime() === s.start.getTime()
...
/** @hidden */
export var getCurrentItemIndex = function (ref, all, ignoreIsAllDay) {
    if (ignoreIsAllDay === void 0) { ignoreIsAllDay = false; }
    var current = ref;
    var sorted = getSortedSlots(all, ignoreIsAllDay);
    var currentIndex = sorted
        .findIndex(function (i) { return compareItems(i, current, ignoreIsAllDay); });
    return {
...
/** @hidden */
export var findCurrentItem = function (ref, all, ignoreIsAllDay) {
    if (ignoreIsAllDay === void 0) { ignoreIsAllDay = false; }
    if (!ref || !all) {
        return null;
    }
    var _a = getCurrentItemIndex(ref, all, ignoreIsAllDay) || {}, currentIndex = _a.currentIndex, sorted = _a.sorted;

...
import { getModelFields, getField, findMaster, setField, toSchedulerGroups, getToday } from './utils/main.js';
import { clone, getListeners, getTemplate, isRtl, templateRendering } from '@progress/kendo-vue-common';
import { SchedulerHeader } from './components/header/SchedulerHeader.js';
import { provideIntlService, provideLocalizationService } from '@progress/kendo-vue-intl';
import { SchedulerNavigation } from './components/header/navigation/SchedulerNavigation.js';
import { addDays, addMonths } from '@progress/kendo-date-math';
import { SchedulerFooter } from './components/footer/SchedulerFooter.js';
import { ToolbarSpacer } from '@progress/kendo-vue-buttons';
import { NavigationDatePicker } from './components/header/navigation/NavigationDatePicker.js';
import { ViewSelectorList } from './components/header/view-selector/ViewSelectorList.js';
import { BusinessHours } from './components/footer/bussiness-hours/BusinessHours.js';
import { DayView } from './views/day/DayView.js';
import { validatePackage } from '@progress/kendo-vue-common';
import { packageMetadata } from './package-metadata.js';
import { AgendaView, MonthView, WeekView, WorkWeekView } from './main.js';
import { TimelineView } from './views/time/TimelineView.js';
...
var SchedulerVue2 = {
  ...
  data: function data() {
    ...
    return {
      ks: {
        ...
        groups: toSchedulerGroups(this.$props.group, this.$props.resources),
        dateRange: dateRange,
  ...
  render: function render(createElement) {
    ...
    var resizeHint = templateRendering.call(this, view.resizeHint || this.$props.resizeHint, {});
    ...
    h(SchedulerHeader, this.v3 ? function () {
      return [h(SchedulerNavigation, {
        isRtl: _this2.isRtl,
    ...
    // @ts-ignore  function children
    h(SchedulerFooter, this.v3 ? function () {
      return [
      // @ts-ignore  function children
      h(BusinessHours, {
    ...
    footerRender = getTemplate.call(this, {
      h: h,
      template: footer,
  ...
  methods: {
      ...
      if (action.series) {
        if (Array.isArray(action.dataItem)) {
          action.dataItem.map(function (dataItem) {
            var masterClone = clone(findMaster(action.dataItem, _this.fields, _this.dataItems));
            var newDataItem = clone(dataItem);
            setField(newDataItem, _this.fields.originalStart, getField(masterClone, _this.fields.originalStart));
            setField(newDataItem, _this.fields.recurrenceId, getField(masterClone, _this.fields.recurrenceId));
            setField(newDataItem, _this.fields.recurrenceExceptions, getField(masterClone, _this.fields.recurrenceExceptions));
            updated.push(newDataItem);
          });
        } else {
          var masterClone = clone(findMaster(action.dataItem, this.fields, this.dataItems));
          var newDataItem = clone(action.dataItem);
          setField(newDataItem, this.fields.originalStart, getField(masterClone, this.fields.originalStart));
          setField(newDataItem, this.fields.recurrenceId, getField(masterClone, this.fields.recurrenceId));
          setField(newDataItem, this.fields.recurrenceExceptions, getField(masterClone, this.fields.recurrenceExceptions));
          updated.push(newDataItem);
        }
      } else {
        if (Array.isArray(action.dataItem)) {
          action.dataItem.map(function (dataItem) {
            var isException = getField(dataItem, _this.fields.recurrenceRule) !== null && getField(dataItem, _this.fields.recurrenceRule) !== undefined;
            var isRecurring = getField(dataItem, _this.fields.recurrenceId) !== null && getField(dataItem, _this.fields.recurrenceId) !== undefined;
            if (isRecurring && isException) {
              var masterClone = clone(findMaster(dataItem, _this.fields, _this.dataItems));
              var exceptionDate = getField(dataItem, _this.fields.originalStart);
              var currentExceptions = getField(masterClone, _this.fields.recurrenceExceptions) || [];
              setField(masterClone, _this.fields.recurrenceExceptions, __spreadArray(__spreadArray([], currentExceptions, true), [exceptionDate], false));
              setField(dataItem, _this.fields.recurrenceRule, null);
          ...
          var isException = getField(action.dataItem, this.fields.recurrenceRule) !== null && getField(action.dataItem, this.fields.recurrenceRule) !== undefined;
          var isRecurring = getField(action.dataItem, this.fields.recurrenceId) !== null && getField(action.dataItem, this.fields.recurrenceId) !== undefined;
          if (isRecurring && isException) {
            var masterClone = clone(findMaster(action.dataItem, this.fields, this.dataItems));
            var exceptionDate = getField(action.dataItem, this.fields.originalStart);
            var currentExceptions = getField(masterClone, this.fields.recurrenceExceptions) || [];
            setField(masterClone, this.fields.recurrenceExceptions, __spreadArray(__spreadArray([], currentExceptions, true), [exceptionDate], false));
            setField(action.dataItem, this.fields.recurrenceRule, null);
            updated.push(masterClone);
    ...
    handleRemove: function handleRemove(action) {
      ...
      if (action.series) {
        ...
        setField(dataItem, this.fields.originalStart, getField(masterClone, this.fields.originalStart));
        setField(dataItem, this.fields.recurrenceId, getField(masterClone, this.fields.recurrenceId));
        setField(dataItem, this.fields.recurrenceRule, getField(masterClone, this.fields.recurrenceRule));
        setField(dataItem, this.fields.recurrenceExceptions, getField(masterClone, this.fields.recurrenceExceptions));
    ...
    handleNextClick: function handleNextClick(event) {
      ...
      var newDate = isMonthView ? addMonths(this.ks.currentDate, Math.round(offset / 27)) : addDays(this.ks.currentDate, offset);
...
export { Scheduler, SchedulerVue2 };
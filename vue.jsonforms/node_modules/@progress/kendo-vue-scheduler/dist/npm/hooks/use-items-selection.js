"use strict";

var __spreadArray = undefined && undefined.__spreadArray || function (to, from, pack) {
  if (pack || arguments.length === 2) for (var i = 0, l = from.length, ar; i < l; i++) {
    if (ar || !(i in from)) {
      if (!ar) ar = Array.prototype.slice.call(from, 0, i);
      ar[i] = from[i];
    }
  }
  return to.concat(ar || Array.prototype.slice.call(from));
};
Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.dispatchSelectedItems = exports.ITEMS_SELECT_ACTION = void 0;
var main_1 = require("../utils/main");
/**
 * Represents the available `action` types for the `SchedulerItemsSelectAction` objects.
 *
 * The available types are:
 * - 'ITEMS_SELECT_SELECT`
 * - 'ITEMS_SELECT_SELECT_NEXT`
 * - 'ITEMS_SELECT_SELECT_PREV`
 * - 'ITEMS_SELECT_RESET`
 * - 'ITEMS_SELECT_ADD`
 * - 'ITEMS_SELECT_REMOVE`
 */
var ITEMS_SELECT_ACTION;
(function (ITEMS_SELECT_ACTION) {
  ITEMS_SELECT_ACTION["select"] = "ITEMS_SELECT_SELECT";
  ITEMS_SELECT_ACTION["selectNext"] = "ITEMS_SELECT_SELECT_NEXT";
  ITEMS_SELECT_ACTION["selectPrev"] = "ITEMS_SELECT_SELECT_PREV";
  ITEMS_SELECT_ACTION["reset"] = "ITEMS_SELECT_RESET";
  ITEMS_SELECT_ACTION["add"] = "ITEMS_SELECT_ADD";
  ITEMS_SELECT_ACTION["remove"] = "ITEMS_SELECT_REMOVE";
})(ITEMS_SELECT_ACTION = exports.ITEMS_SELECT_ACTION || (exports.ITEMS_SELECT_ACTION = {}));
/** @hidden */
var dispatchSelectedItems = function dispatchSelectedItems(action, event) {
  switch (action.type) {
    case ITEMS_SELECT_ACTION.select:
      if (!action.item) {
        return;
      }
      this.bv.selectedItems = [action.item];
      break;
    case ITEMS_SELECT_ACTION.selectNext:
      {
        if (!action.item || !this.bv.items) {
          return;
        }
        var next = (0, main_1.findNextItem)(action.item, this.bv.items, action.ignoreIsAllDay, false);
        if (next) {
          event.preventDefault();
          this.bv.selectedItems = [next];
        }
        break;
      }
    case ITEMS_SELECT_ACTION.selectPrev:
      {
        if (!action.item || !this.bv.items) {
          return;
        }
        var next = (0, main_1.findNextItem)(action.item, this.bv.items, action.ignoreIsAllDay, true);
        if (next) {
          event.preventDefault();
          this.bv.selectedItems = [next];
        }
        break;
      }
    case ITEMS_SELECT_ACTION.reset:
      this.bv.selectedItems = [];
      break;
    case ITEMS_SELECT_ACTION.add:
      if (!action.item) {
        return;
      }
      if (!this.bv.selectedItems.some(function (si) {
        return si === action.item;
      })) {
        this.bv.selectedItems = __spreadArray(__spreadArray([], this.bv.selectedItems, true), [action.item], false);
      }
      break;
    case ITEMS_SELECT_ACTION.remove:
      if (!action.item) {
        return;
      }
      this.bv.selectedItems = __spreadArray([], this.bv.selectedItems.filter(function (si) {
        return si !== action.item;
      }), true);
      break;
    default:
      break;
  }
};
exports.dispatchSelectedItems = dispatchSelectedItems;
...
exports.getSortedSlots = exports.compareItems = exports.findFirstItem = exports.noop = exports.mapItemsToSlots = exports.mapSlotsToItems = exports.calculateOrder = exports.findMissing = exports.shallowCompare = exports.extractResources = exports.extractResourceFields = exports.extractGroups = exports.setTime = exports.isNullOrEmptyString = exports.isBlank = exports.generateResourceFields = exports.toSchedulerResources = exports.toSchedulerGroups = exports.isInTimeRange = exports.getModelFields = exports.defaultModelFields = exports.roundAllDayEnd = exports.intersects = exports.isException = exports.buildException = exports.isMaster = exports.findMaster = exports.groupResources = exports.isRecurring = exports.isRecurringException = exports.toInvariantTime = exports.toUTCDateTime = exports.getUTCDate = exports.toUTCDate = exports.isRecurringMaster = exports.isPresent = exports.assignFields = exports.assignField = exports.setField = exports.getField = exports.slotDive = exports.getToday = exports.dateWithTime = exports.addUTCDays = exports.formatEventTime = exports.isGroupped = exports.isMultiDay = exports.last = exports.first = exports.COLLECTION_ACTION = void 0;
exports.orderSort = exports.findNextItem = exports.findCurrentItem = exports.getCurrentItemIndex = exports.compareSlots = void 0;
 ...
 * @hidden
 */
var getField = function (obj, field) { return (0, kendo_vue_common_1.getter)(field)(obj); };
...
var setField = function (obj, field, value) {
    ...
    return (0, kendo_vue_common_1.setter)(field)(obj, value);
};
...
var isRecurring = function (event, fields) {
    var recurrenceId = (0, kendo_vue_common_1.getter)(fields.recurrenceId || 'recurrenceId')(event);
    var recurrenceRule = (0, kendo_vue_common_1.getter)(fields.recurrenceRule || 'recurrenceRule')(event);
...
var buildException = function (event, fields, data) {
    ...
    var copy = (0, kendo_vue_common_1.clone)(event);
    assignField(copy, headEvent, fields.id);
    if (fields.id !== undefined) {
        (0, exports.setField)(copy, fields.id, main_1.DEFAULT_ID);
    }
...
var roundAllDayEnd = function (_a) {
    ...
    return endDate.getTime() !== end.getTime() ||
        startDate.getTime() === endDate.getTime() ? endDate.addDays(1) : endDate;
};
...
var isInTimeRange = function (date, min, max) {
    return ((min.getHours() < date.getHours()) ||
        (min.getHours() === date.getHours() && min.getMinutes() <= date.getMinutes()))
        && ((date.getHours() < max.getHours()) ||
            (max.getHours() === date.getHours() && date.getMinutes() < max.getMinutes()));
};
...
var isNullOrEmptyString = function (value) {
    return (0, exports.isBlank)(value) || (value.trim && value.trim().length === 0);
};
...
var setTime = function (origin, candidate) {
    var date = (0, kendo_vue_common_1.cloneDate)(origin);
    if (date) {
...
var calculateOrder = function (current, items, slots, ignoreIsAllDay) {
    ...
    slots.forEach(function (slot) {
        ...
        sorted.forEach(function (item) {
            var fit = (ignoreIsAllDay || (item.$props.isAllDay === slot.cSlot.isAllDay))
                && item.$props.range.index === slot.cSlot.range.index
                && item.$props.group.index === slot.cSlot.group.index
                && (0, exports.intersects)(item.$props.start, item.$props.end, slot.cSlot.start, slot.cSlot.end);
            if (fit) {
...
var mapSlotsToItems = function (items, slots, ignoreIsAllDay) {
    ...
    slots.forEach(function (slot) {
        var orderNumbers = [];
        items.forEach(function (item) {
            var currentSlot = slot.cSlot ? slot.cSlot : slot;
            var fit = (ignoreIsAllDay || (item.isAllDay === currentSlot.isAllDay))
                && item.range.index === currentSlot.range.index
                && item.group.index === currentSlot.group.index
                && (0, exports.intersects)(item.start, item.end, currentSlot.start, currentSlot.end);
            if (fit) {
                var order = (0, exports.findMissing)(orderNumbers);
...
/** @hidden */
var compareItems = function (f, s, ignoreIsAllDay) { return Boolean(f.uid === s.uid
    && ((f.occurrenceId === undefined) ||
        f.occurrenceId === s.occurrenceId)
    && (f.group.index === s.group.index)
    && (f.range.index === s.range.index)
    && (ignoreIsAllDay || f.isAllDay === s.isAllDay)); };
exports.compareItems = compareItems;
/** @hidden */
var getSortedSlots = function (all, ignoreIsAllDay) {
    var filtered = all.filter(function (i) { return i.element !== null; });
    var sorted = (0, kendo_data_query_1.orderBy)(filtered, [
        { field: 'group.index', dir: 'asc' },
        { field: 'range.index', dir: 'asc' },
        (ignoreIsAllDay ? { field: '' } : { field: 'isAllDay', dir: 'desc' }),
        { field: 'start', dir: 'asc' }
    ]);
    return sorted;
};
exports.getSortedSlots = getSortedSlots;
/** @hidden */
var compareSlots = function (f, s) { return Boolean(f.start.getTime() === s.start.getTime()
    && f.end.getTime() === s.end.getTime()
    && f.index === s.index
    && f.group.index === s.group.index
    && f.range.index === s.range.index
    && f.isAllDay === s.isAllDay); };
exports.compareSlots = compareSlots;
/** @hidden */
var getCurrentItemIndex = function (ref, all, ignoreIsAllDay) {
    if (ignoreIsAllDay === void 0) { ignoreIsAllDay = false; }
    var current = ref;
    var sorted = (0, exports.getSortedSlots)(all, ignoreIsAllDay);
    var currentIndex = sorted
        .findIndex(function (i) { return (0, exports.compareItems)(i, current, ignoreIsAllDay); });
    return {
        currentIndex: currentIndex,
        sorted: sorted
    };
};
exports.getCurrentItemIndex = getCurrentItemIndex;
/** @hidden */
var findCurrentItem = function (ref, all, ignoreIsAllDay) {
    if (ignoreIsAllDay === void 0) { ignoreIsAllDay = false; }
    if (!ref || !all) {
        return null;
    }
    var _a = (0, exports.getCurrentItemIndex)(ref, all, ignoreIsAllDay) || {}, currentIndex = _a.currentIndex, sorted = _a.sorted;
    var next = sorted[currentIndex];
    return next;
};
exports.findCurrentItem = findCurrentItem;
/** @hidden */

...
var toSlots = function (_dateRange, _a, _b) {
    ...
    groups.forEach(function (group) {
        ranges.forEach(function (range) {
            ...
            for (var current = viewStart.clone(), index = 0, deficiency = offset < 0 ? offset * main_1.MS_PER_MINUTE * -1 : 0; current.getTime() < viewEnd.getTime(); index++, current = current.addTime(step)) {
                var refTo = {};
                var zonedStart = current.clone();
                var zonedEnd = current.addTime(step);
                var difference = (zonedEnd.timezoneOffset - zonedStart.timezoneOffset) * main_1.MS_PER_MINUTE;

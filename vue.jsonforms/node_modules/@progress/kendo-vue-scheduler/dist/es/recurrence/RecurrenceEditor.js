...
var RecurrenceEditorVue2 = {
  ...
  computed: {
    rrule: function rrule() {
      return getRule(this.$props.value);
    },
    frequencies: function frequencies() {
      return getFrequencies(this.localizeMessage);
    },
    frequency: function frequency() {
      return getFrequency(this.rrule);
    },
    extendedWeekDays: function extendedWeekDays() {
      return getExtendedWeekDays(this.intl, this.localizeMessage);
    },
    weekDay: function weekDay() {
      return getWeekDay(this.extendedWeekDays, this.rrule, this.$props.start);
    },
    months: function months() {
      return getMonths(this.intl);
    },
    month: function month() {
      return getMonth(this.months, this.rrule);
    },
    offsets: function offsets() {
      return getOffsets(this.localizeMessage);
    },
    offset: function offset() {
      return getOffset(this.offsets, this.rrule);
    },
    endRule: function endRule() {
      return getEndRule(this.rrule);
    },
    monthDay: function monthDay() {
      return getMonthDay(this.rrule, this.$props.start);
    },
    until: function until() {
      return getUntil(this.rrule);
    },
    weekDays: function weekDays() {
      return getWeekDays(this.intl);
    },
    repeatOnRule: function repeatOnRule() {
      return getRepeatOnRule(this.rrule);
    }
  },
  data: function data() {
    return {
      defaultCount: 1,
      defaultUntil: ZonedDate.fromUTCDate(this.$props.start)
    };
  },
  // @ts-ignore
  ...
  render: function render(createElement) {
    ...
    var yearlyWeekDay = function yearlyWeekDay() {
      ...
      }, [this.localizeMessage(recurrenceEditorYearlyOf, true)]), h(DropDownList, {
    ...
    var count = function count() {
      ...
      h(Label, {
        ...
        return [_this.localizeMessage(recurrenceEditorEndOccurrence, true)];
      } : [_this.localizeMessage(recurrenceEditorEndOccurrence, true)])];
    };
      ...
      return (
        ...
        h(DatePicker, {
          ...
          attrs: this.v3 ? undefined : {
            ...
            value: that.endRule === 'until' ? that.until : toLocalDate(that.defaultUntil)
          },
          disabled: that.endRule !== 'until',
          min: that.$props.start,
          value: that.endRule === 'until' ? that.until : toLocalDate(that.defaultUntil),
  ...
  methods: {
    localizeMessage: function localizeMessage(baseMassage, removePrepend) {
      ...
      return this.localization.toLanguageString(message, messages[message]);
    },
    onChangeHandler: function onChangeHandler(newRrule) {
      if (newRrule.freq === 'never') {
        ...
        this.$emit('change', {
          value: serializeRule(newRrule, this.$props.timezone)
        });
      }
    },
    handleFrequencyChange: function handleFrequencyChange(newFreq) {
      ...
      if (newFreq === 'weekly') {
        newRrule.byWeekDay = [{
          day: ZonedDate.fromLocalDate(this.$props.start, this.timezone).getDay(),
          offset: 0
        }];
      }
    ...
    handleUntilChange: function handleUntilChange(event) {
      ...
      var value = ZonedDate.fromLocalDate(newUntil, this.$props.timezone);
      this.onChangeHandler(Object.assign({}, this.rrule, {
        until: value
      }));
    ...
    handleRepeatOnRuleChange: function handleRepeatOnRuleChange(event) {
      ...
      if (event.value === 'monthday') {
        ...
        if (typeof this.weekDay.value === 'string') {
          /* day, weekday or weekend */
          newRrule.bySetPosition = [this.offset.value];
          newRrule.byWeekDay = weekDayRuleFromString(this.weekDay.value);
        } else {
          /* specific weekday */
          newRrule.byWeekDay = [{
...
export { RecurrenceEditor, RecurrenceEditorVue2 };
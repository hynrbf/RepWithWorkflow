...
export default defineComponent({
  name: "EmployeeOwnersAndControllers",
  components: {
    EmployeeCorporateControllerItemComponent: defineAsyncComponent(
      () =>
        import(
          "@/pages/employee-pages/owners-and-controllers/partials/EmployeeCorporateControllerItemComponent.vue"
        ),
    ),
    EmployeeIndividualControllerItemComponent: defineAsyncComponent(
      () =>
        import(
          "@/pages/employee-pages/owners-and-controllers/partials/EmployeeIndividualControllerItemComponent.vue"
        ),
    ),
    EmployeeIndividualControllerChartModal: defineAsyncComponent(
      () =>
        import(
          "@/pages/employee-pages/owners-and-controllers/partials/EmployeeIndividualControllerChartModal.vue"
        ),
    ),
    EmployeeCorporateControllerChartModal: defineAsyncComponent(
      () =>
        import(
          "@/pages/employee-pages/owners-and-controllers/partials/EmployeeCorporateControllerChartModal.vue"
        ),
    ),
  },
  data() {
  ...
  computed: {
    ...
    isNextButtonDisabled(): boolean {
      return (
        this.registeredEmployee.noOfIndividualShareholders < 1 &&
        this.registeredEmployee.noOfCorporateShareholders < 1
      );
    },
  ...
  created() {
    this.registeredEmployee =
      this.appService.getCachedEmployee() ?? new Employee();

    if (!this.registeredEmployee.email) {
      throw new Error(
        "Customer object must be already been cached before going to owners and controllers screen",
      );
    }

    this.firmName = this.appService.getEmployeeFirmName();

    this.individualControllersCount =
      this.registeredEmployee.noOfIndividualShareholders;
    this.setupIndividualControllerItems();

    this.corporateControllersCount =
      this.registeredEmployee.noOfCorporateShareholders;
    ...
    this.pageDescription = `The FCA require specific information about all
    Individual/Corporate Owners (Shareholders) and Controllers, who own or control 10% or more of the capital or voting
    rights of ${this.firmName}.`;
    this.tabs = [
      {
  ...
  unmounted() {
    ...
    this.eventBus.off(AppConstants.formFieldChangedEvent);
  ...
  methods: {
    onTabItemSelect(e: TabStripSelectEventArguments) {
      if (this.selectedControllerTabIndex === 0 && e.selected > 1) {
        if (
          !(
            this.registeredEmployee.individualControllers &&
            this.registeredEmployee.individualControllers.length > 0
          )
        ) {
          return;
        }
      ...
      if (this.selectedControllerTabIndex === 1 && e.selected > 1) {
        if (
          !(
            this.registeredEmployee.corporateControllers &&
            this.registeredEmployee.corporateControllers.length > 0
          )
        ) {
          return;
        }
    ...
    async saveInfoAsync(
      ...
      const updatedCustomer =
        await this.organizationalStructureService.getEmployeeByEmailAsync(
          this.registeredEmployee?.email ?? "",
        );
      ...
      updatedCustomer.individualControllers =
      ...
      await this.organizationalStructureService
        ...
        .then((customer) => {
          ...
          this.individualControllersCount = customer.noOfIndividualShareholders;
          this.corporateControllersCount = customer.noOfCorporateShareholders;
...
<template src="./employee-owners-and-controllers.html" />

<style src="./employee-owners-and-controllers.css" />
...
import EmployeeFcaRoleItemTemplate
  from "@/pages/employee-pages/organizational-structure/partials/EmployeeFcaRoleItemTemplate.vue";
import EmployeeFcaRoleHeaderTemplate
  from "@/pages/employee-pages/organizational-structure/partials/EmployeeFcaRoleHeaderTemplate.vue";

export default defineComponent({
  components: {
    KendoDropdownListWithTypeComponent,
    DynamicAvatar,
    EmployeeFcaRoleItemTemplate,
  ...
  computed: {
    ...
    isMinimumFieldsSupplied(): boolean {
      if (!(this.employeeInternal?.firstName &&
  ...
  methods: {
    ...
    setUniqueIdentifier(value: string): string {
      const identifier = `${AppConstants.employeeOrganizationalStructureRoute}-addStaffModal${value}`;

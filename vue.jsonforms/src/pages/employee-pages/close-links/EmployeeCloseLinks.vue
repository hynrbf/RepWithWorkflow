...
export default defineComponent({
  ...
  data() {
    return {
      ...
      // Temp
      currentEmployeeCustomer: undefined as Employee | undefined,
      currentEmployeeFirmName: "",
    };
  },
  setup() {
  ...
  created() {
    if (this.currentArCustomer?.closeLinks) {
      const closeLinks = this.mapperService.mapCloseLinkEntitiesToModels(
        this.currentArCustomer.closeLinks,
      );
      ...
      this.hasOtherCloseLinks = this.collapsibleCloseLinkItems.length > 0;
    ...
    this.natureOfCloseLink = [
      `Parent Undertaking of ${this.currentArFirmName}`,
      `Subsidiary Undertaking of ${this.currentArFirmName}`,
      `Parent Undertaking of a Subsidiary Undertaking of ${this.currentArFirmName}`,
      `Subsidiary Undertaking of a Parent Undertaking of ${this.currentArFirmName}`,
      `Undertaking of which ${this.currentArFirmName} owns or controls 20% or more of the capital or voting rights`,
    ];
  ...
  methods: {
    ...
    submit(isAutoNext: boolean) {
      ...
      (this.$refs["formElement"] as any)?.$el?.requestSubmit?.();
    },
    ...
    async saveInfoAsync(
      ...
      await this.updateCustomerArByEmailAsync(customerEmail, {
    ...
    addCloseLink() {
      this.collapsibleCloseLinkItems.forEach(
        (item) => (item.isCollapsed = true),
      );

      for (const item of this.collapsibleCloseLinkItems) {
    ...
    async goToOwnersAndControllersAsync() {
      const activeRouteServiceRouteSequenceNo =
        this.navigationService.getActiveRouteServiceRouteSequenceNo(
          AppConstants.arOwnersAndControllersRoute,
        );
    ...
    setUniqueIdentifier(value: string): string {
      const identifier = `${AppConstants.employeeCloseLinkRoute}${value}`;
...
<template src="./employee-close-links.html" />

<style scoped src="./employee-close-links.css" />
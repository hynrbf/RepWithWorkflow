...
export default defineComponent({
  name: "ARProfessionalIndemnityInsurance",
  components: {
  ...
  data() {
    return {
      ...
      activePiiTabIndex: 0,
      tempDetails: new FirmBasicInfo(),
      tempIndex: 0,
      kendoDialogInstanceConfirmInsurer:
        null as KendoActionDialogComponent | null,
      kendoDialogInstanceConfirmBroker:
        null as KendoActionDialogComponent | null,
      kendoDialogInstanceErrorInsurerIsTheSame:
        null as KendoActionDialogComponent | null,
      kendoDialogInstanceErrorBrokerIsTheSame:
        null as KendoActionDialogComponent | null,
    };
  },
  setup() {
    ...
    piiTabs: {
      get() {
        return this.professionalIndemnityItems.map(({ id, pii }, index) => ({
          id: `pii-${id}`,
          title: pii.insurerName ?? "New Insurer",
          content: "",
  ...
  created() {
    ...
    const selectedCompany = this.arCustomer.selectedCompany as CompanyEntity;
  ...
  mounted() {
    try {
      ...
      if (!(this.arCustomer.professionalIndemnities?.length > 0)) {
        this.addInitialProfessionalIndemnityItem();
        return;
      }

      professionalIndemnities = this.arCustomer.professionalIndemnities;
      ...
      if (this.$refs.kendoActionDialogConfirmInsurer) {
        this.kendoDialogInstanceConfirmInsurer = this.$refs
          .kendoActionDialogConfirmInsurer as KendoActionDialogComponent;
      }

      if (this.$refs.kendoActionDialogConfirmBroker) {
        this.kendoDialogInstanceConfirmBroker = this.$refs
          .kendoActionDialogConfirmBroker as KendoActionDialogComponent;
      }

      if (this.$refs.kendoErrorInsurerIsTheSame) {
        this.kendoDialogInstanceErrorInsurerIsTheSame = this.$refs
          .kendoErrorInsurerIsTheSame as KendoActionDialogComponent;
      }

      if (this.$refs.kendoErrorBrokerIsTheSame) {
        this.kendoDialogInstanceErrorBrokerIsTheSame = this.$refs
          .kendoErrorBrokerIsTheSame as KendoActionDialogComponent;
      }
    }
  },
  unmounted() {
  ...
  methods: {
    ...
    onPiiInsurerDetailUpdated(
      ...
      if (piiInsurer.firmName === this.firmName) {
        this.kendoDialogInstanceErrorInsurerIsTheSame?.setActionDialogMessage(
          "Error",
        );
        return;
      }

      if (!confirm && !(piiInsurer.fcaStatus === "Authorised")) {
        this.tempDetails = piiInsurer;
        this.tempIndex = index;
        this.kendoDialogInstanceConfirmInsurer?.setActionDialogMessage(
          "Confirm",
        );
        return;
      }
    ...
    onPiiBrokerDetailUpdated(
      piiBroker: FirmBasicInfo,
      index: number,
      confirm?: boolean,
    ) {
      if (piiBroker.firmName === this.firmName) {
        this.kendoDialogInstanceErrorBrokerIsTheSame?.setActionDialogMessage(
          "Error",
        );
        return;
      }

      if (!confirm && !(piiBroker.fcaStatus === "Authorised")) {
        this.tempDetails = piiBroker;
        this.tempIndex = index;
        this.kendoDialogInstanceConfirmBroker?.setActionDialogMessage(
          "Confirm",
        );
        return;
      }
    ...
    confirmInsurer() {
      this.onPiiInsurerDetailUpdated(this.tempDetails, this.tempIndex, true);
      this.kendoDialogInstanceConfirmInsurer?.closeActionDialog();
    },

    confirmBroker() {
      this.onPiiBrokerDetailUpdated(this.tempDetails, this.tempIndex, true);
      this.kendoDialogInstanceConfirmBroker?.closeActionDialog();
    },
    ...
    handleSubmit(isAutoNext: boolean) {
      ...
      if (isAutoNext) {
        useAlert({
          title: "Confirm",
          content:
            "Appointed Representative Professional Indemnity Insurance (PII) is incomplete. Are you sure you wish to proceed?",
    ...
    setUniqueIdentifier(value: string): string {
      const identifier = `${AppConstants.arProfessionalIndemnityRoute}${value}`;
...
<template src="./ar-professional-indemnity-insurance.html" />

<style scoped src="./ar-professional-indemnity-insurance.css" />

...
import { useArFinancialPromotionStore } from "@/stores/financial-promotion/useArFinancialPromotionStore";
...
export default defineComponent({
  components: {
    ViewOutletModal: defineAsyncComponent(
      () => import("./partials/ARViewOutletModal.vue")
    ),
    AddOutletModal: defineAsyncComponent(
      () => import("./partials/ARAddOutletModal.vue")
    ),
    AddPromotionModal: defineAsyncComponent(
      () => import("./partials/ARAddPromotionModal.vue")
    ),
    OutletActions: defineAsyncComponent(
      () => import("./partials/AROutletActions.vue")
    ),
    PromotionActions: defineAsyncComponent(
      () => import("./partials/ARPromotionActions.vue")
    ),
    ViewPromotionModal: defineAsyncComponent(
      () => import("./partials/ARViewPromotionModal.vue")
    ),
    MediaDropdown: defineAsyncComponent(
      () => import("./partials/ARMediaDropdown.vue")
    ),
    PromotionTableFilters: defineAsyncComponent(
      () => import("./partials/ARPromotionTableFilters.vue")
    ),
  },
  data() {
    return {
  ...
  computed: {
    ...
    ...mapState(useArCustomerStore, ["currentArFirmName", "currentArCustomer"]),
    ...mapState(useArMediaMarketingOutletStore, [
      "mediaMarketingOutlet",
      "mediaMarketingOutlets",
      "mediaMarketingOutletsArchived",
      "mediaMarketingOutletsUnarchived",
    ]),
    ...mapState(useArFinancialPromotionStore, [
      "financialPromotion",
      "financialPromotions",
    ...
    promotionTypeOptions(): Array<{
      ...
      return [
        {
          label: `${this.currentArFirmName} Products/Services`,
          value: "self",
        },
        {
  ...
  methods: {
    ...
    ...mapActions(useArMediaMarketingOutletStore, [
    ...
    ...mapActions(useArFinancialPromotionStore, [
    ...
    async addOutletAsync(payload: Partial<MediaMarketingOutlet>) {
      ...
      try {
        ...
        this.webScrapeService.registerMediaAsync(
          ...
          `${this.currentArCustomer.id}`
        );
        setTimeout(() => {
          this.fetchFinancialPromotionsAsync();
        }, 2000);
      } catch (error) {
        useNotification({
          type: NotificationType.ERROR,
          content: "Error Adding Media Marketing Outlet.",
        });
      }
    },
    ...
    setUniqueIdentifier(value: string): string {
      const identifier = `${AppConstants.arMarketingAndFinancialPromotionsRoute}${value}`;
...
<template src="./ar-marketing-and-financial-promotions.html" />

<style scoped src="./ar-marketing-and-financial-promotions.scss" />
